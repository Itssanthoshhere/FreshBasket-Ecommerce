...
const [wishlist, setWishlist] = useState<string[]>([]);
  ...
  return (
    <>
      ...
      {/* POPUP MODAL */}
      <div
        className={`fixed inset-0 z-99999 flex items-center justify-center bg-black/40 backdrop-blur-sm top-0 left-0 transition-opacity duration-300 ${openModal ? "opacity-100 visible" : "opacity-0 invisible"}`}
      >
        <div
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              {selectedProduct?.lessprice && (
                <div className="font-semibold line-through text-gray-500 text-md">
                  Rs. {selectedProduct.lessprice?.toString()?.replace(/Rs\.?/i, "").trim()}
                </div>
              )}
            </div>
